...
export default function TerminalShell({ children }: { children: React.ReactNode }) {
  return <div className="min-h-screen bg-[var(--bg)] text-[var(--fg)]">
    ...
    <main className="md:pl-64"><header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-[var(--line)] bg-[var(--bg)]/90 px-5 backdrop-blur"><Link href="/discover" className="flex items-center gap-2 text-sm font-semibold md:hidden"><FlowMark compact />FLOW</Link><div className="mono text-[10px] text-[var(--muted)]">FLOW / MAINNET</div><div className="flex items-center gap-2"><ThemeToggle/><div className="md:hidden"><WalletStatus/></div></div></header><div className="pb-24">{children}</div><nav className="fixed bottom-0 left-0 right-0 z-20 grid grid-cols-4 border-t border-[var(--line)] bg-[var(--bg)]/95 backdrop-blur md:hidden"><Link href="/discover" className="py-3 text-center text-[10px]">Discover</Link><Link href="/watch" className="py-3 text-center text-[10px]">Watch</Link><Link href="/autopilot" className="py-3 text-center text-[10px]">Auto</Link><Link href="/connect/fund" className="py-3 text-center text-[10px]">Wallet</Link></nav></main>
  </div>;
}