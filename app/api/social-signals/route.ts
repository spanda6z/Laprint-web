...
export async function GET(req:NextRequest){
 ...
 try{
  ...
  const signals=(body.data||[] as XPost[]).map((post:XPost)=>{const author=post.author_id?users.get(post.author_id):undefined; const m=post.public_metrics||{}; return {type:"mention",postId:post.id,text:post.text,createdAt:post.created_at||null,author:author?{id:author.id,name:author.name||author.username||"Unknown",username:author.username||"",image:author.profile_image_url||null}:null,url:author?.username?"https://x.com/"+author.username+"/status/"+post.id:null,engagement:(m.like_count||0)+(m.reply_count||0)+(m.retweet_count||0),metrics:m}});
