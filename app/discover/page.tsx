  ...
  return <main className="min-h-screen bg-[var(--bg)] text-[var(--fg)]">
    <header className="sticky top-0 z-20 border-b border-[var(--line)] bg-[var(--bg)]/90 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 md:px-8">
        <Link href="/" className="font-semibold tracking-tight">La😂Print</Link>
    ...
    <section className="mx-auto max-w-7xl px-5 pb-10 pt-12 md:px-8 md:pt-16">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {tokens.map(t=>{const total=t.buys5m+t.sells5m;const buy=total?Math.round(t.buys5m/total*100):0;return <article key={t.address} className="group rounded-[26px] border border-[var(--line)] bg-[var(--panel)] p-5 transition hover:-translate-y-0.5 hover:border-[var(--line-strong)]">
          ...
          <div className="mt-6 flex items-end justify-between"><div><div className="text-2xl font-semibold tracking-tight">{price(t.priceUsd)}</div><div className="mt-1 text-[10px] text-[var(--muted)]">LIVE PRICE</div></div><div className="text-right"><div className="text-sm font-semibold">{t.change24h>=0?"+":""}{t.change24h.toFixed(1)}%</div><div className="mono text-[9px] text-[var(--muted)]">24H</div></div></div>
          <div className="mt-5"><div className="flex justify-between text-[9px] text-[var(--muted)]"><span>BUY FLOW · 5M</span><span>{buy}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--line)]"><div className="h-full bg-[var(--fg)]" style={{width:buy+"%"}}/></div></div>
