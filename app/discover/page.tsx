    ...
    <section className="mx-auto max-w-7xl px-5 pb-10 pt-12 md:px-8 md:pt-16">
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {tokens.map(t=>{const total=t.buys5m+t.sells5m;const buy=total?Math.round(t.buys5m/total*100):0;return <article key={t.address} className="group rounded-3xl border border-[var(--line)] bg-[var(--panel)] p-5 transition hover:-translate-y-0.5 hover:border-[var(--line-strong)]">
          ...
          <div className="mt-5 flex gap-2"><Link href={"/token?address="+encodeURIComponent(t.address)+"&symbol="+encodeURIComponent(t.symbol)} className="flex-1 rounded-full bg-[var(--fg)] px-4 py-3 text-center text-xs font-bold text-[var(--bg)]">View token</Link><Link href={"/trade?token="+encodeURIComponent(t.address)+"&symbol="+encodeURIComponent(t.symbol)} className="rounded-full border border-[var(--line-strong)] px-4 py-3 text-xs">Trade</Link></div>
        </article>})}
      </div>
