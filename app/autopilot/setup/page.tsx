  ...
  return (
    <main className="min-h-screen bg-[#070707]">
      <div className="mx-auto max-w-3xl px-5 py-10">
        ...
        <h1 className="mt-4 text-5xl font-semibold">Configure {tier}.</h1>
          ...
          {tier === "leverage" && (
            <>
              ...
              <div className="mt-8 grid grid-cols-2 gap-3">
                ...
                <div className="rounded-xl border border-white/10 p-4">
                  <span className="text-xs text-zinc-600">MAX LOSS</span>
                  <div className="mt-2 text-sm text-zinc-400">User configurable</div>
                </div>
              </div>
            </>
          )}

          <button className="mt-8 w-full rounded-xl bg-white px-5 py-4 text-sm font-bold text-black">
            Save configuration
          </button>
        </div>
      </div>
...
export default function Setup() {
  return (
    <Suspense fallback={
      <main className="min-h-screen bg-[#070707] p-8 text-zinc-500">Loading setup…</main>
    }>
      <SetupContent />
    </Suspense>
  );
